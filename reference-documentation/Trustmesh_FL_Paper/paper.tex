...
\begin{figure}[t]
    \centering
    \figplaceholder{1.8cm}{%
        TrustMesh-FL three-layer architecture showing the Network Management,
        Computation, and Perception layers, augmented with the FL-specific components
        introduced in this work: \texttt{aggregation-request-tp},
        \texttt{aggregation-confirmation-tp}, \texttt{validation-dataset-distributor},
        \texttt{aggregation\_event\_handler}, and \texttt{federated\_response\_manager}.%
    }
    \caption{%
...
\begin{figure*}[t]
    \centering
    \figplaceholder{3.5cm}{%
        Sequence diagram of the two-phase \tmfl{} protocol.
        Phase~1 (left): IoT node submits training data; scheduling pipeline assigns task
        to compute node; compute node executes training container and returns weights via
        ZMQ.
        Phase~2 (right): IoT node submits trained weights via aggregation-request-tp;
        aggregator compute node collects contributions, computes FedAvg, and submits
        confirmation transaction; aggregation-confirmation-tp on every validator
        independently re-executes FedAvg, runs the four-stage validation pipeline, and
        achieves PBFT consensus; confirmed global model is broadcast to IoT nodes via ZMQ.%
    }
    \caption{Two-phase \tmfl{} protocol sequence diagram.}
    \label{fig:protocol}
\end{figure*}
...
authenticated ZMQ channel. In round~1, weights are initialised randomly; in subsequent
rounds, the IoT node supplies the most recently received global model weights as
initialisation, implementing warm-start local training.
...
all validators must independently construct the same identifier without coordination outside
of consensus. An aggregator compute node is selected by all validators independently from
the same resource snapshot in Redis, with node identifiers serving as a tiebreaker to
guarantee identical selection. The round is assigned status \textsc{collecting}.
...
round. Model weights are stored in CouchDB by the designated aggregator's event handler;
only the CouchDB document identifiers, SHA-256 content hashes, sample counts, and IoT node
connection details are recorded in blockchain state. A configurable collection window
(default: 180 seconds) allows nodes to join the round asynchronously.
...
sample counts, and IoT node ZMQ connection details), and selects the aggregator node
deterministically using resource snapshots from Redis. Model weights are not stored in
blockchain state by this processor; instead, it emits an \texttt{aggregation-request}
event carrying the weight payload, which the designated aggregator's event handler
receives and persists to CouchDB.
...
the node is the designated aggregator, the handler stores the submitted weights in CouchDB
under the document identifier recorded in the request transaction, then starts a collection
timer. On timer expiry, the handler locks the round, fetches all accumulated weight
documents, computes FedAvg, and submits the confirmation transaction. On receiving an
...
than in blockchain state, as the JSON-serialised size of a model (approximately 240\,KB for
...
\midrule
MNISTNet   & 62\,006  & $\approx$240\,KB  & $\approx$1.4\,MB \\
CIFAR10Net & 290\,074 & $\approx$1\,120\,KB & $\approx$6.7\,MB \\
\bottomrule
\end{tabular}
\vspace{2pt}
...
IoT deliveries $= 2N + 1$ model-weight messages. JSON serialisation is approximately
...
approximately 62\,000 parameters designed for 28$\times$28 greyscale inputs.
CIFAR10Net extends this with batch normalisation and a wider feature hierarchy for
32$\times$32 RGB inputs, totalling approximately 290\,000 parameters. Any architectural
...
\textbf{Aggregator selection via Redis.} The aggregator is selected from resource data
stored in Redis, which is external to blockchain consensus. If validators observe
inconsistent Redis state due to replication lag, they may select different aggregators.
In practice, the effect is bounded: the round identifier is derived from workflow
identifier and global round counter, both of which are in blockchain state, so any
disagreement on aggregator identity produces a confirmation transaction that fails
the proposer-identity check and is rejected, triggering a new round.
