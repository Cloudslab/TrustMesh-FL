...
\begin{figure}[t]
    ...
    % ---- Computation Layer ----
    \node[comp, below=9mm of depclient.south west, anchor=north west] (validator) {Sawtooth Validator\\(PBFT)};
    ...
    % ---- Perception Layer ----
    \node[comp, below=23mm of taskexec.south west, anchor=north west] (simulation) {FL Simulation\\(two-phase driver)};
...
\begin{figure*}[t]
    ...
    \node[flactor] (reqtp) at (9.0,0)  {aggregation-request-tp\\(all validators)};
    \node[flactor] (aggh)  at (12.2,0) {Aggregator\\Event Handler};
    \node[flactor] (conftp)at (15.2,0) {aggregation-confirmation-tp\\(all validators)};
    ...
    \draw[msg] ($(reqtp.south)+(0,-4.0)$) -- ++(1.2,0) node[right, font=\tiny, align=left]
        {elect aggregator (round-robin,\\ peer-registry state); metadata $\to$ state;\\ weights $\to$ CouchDB (async)};
    \draw[msg] ($(reqtp.south)+(0,-5.0)$) -- ($(aggh.south)+(0,-5.0)$)   node[note] {aggregation-request event};
    \draw[msg] ($(aggh.south)+(0,-5.6)$)  -- ++(1.05,0) node[right, font=\tiny, align=left]
    ...
    \draw[msg] ($(conftp.south)+(0,-7.2)$) -- ++(1.0,0) node[right, font=\tiny, align=left]
...
\textcolor{red}{[TBD-CONVERGENCE: convergence_accuracy figure, per-alpha convergence
narrative, rounds-to-threshold numbers]}

\subsection{Overhead of Consensus-Validated Aggregation (RQ2)}
\label{sec:eval-overhead}

\textcolor{red}{[TBD-OVERHEAD: timing_breakdown + overhead figures, X\% overhead
number, phase decomposition narrative]}

\subsection{Byzantine Attack Detection (RQ3)}
\label{sec:eval-byzantine}

\textcolor{red}{[TBD-BYZANTINE: table_byzantine, detection rates per attack class,
