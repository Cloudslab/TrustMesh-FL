...
pipelines. BlockFL~\cite{kim2020blockchained} logs local model updates on-chain, and
DeepChain~\cite{weng2021deepchain} records gradient commitments to support incentive
mechanisms and post-hoc auditing; committee-based designs such as BFLC~\cite{li2021bflc}
and VFChain~\cite{peng2022vfchain} delegate aggregation to a small elected group of
nodes. These approaches improve accountability, but the blockchain primarily provides an
immutable record of what was \emph{submitted}, not a verification of what was
\emph{computed}. Aggregation remains the responsibility of a designated entity or a
small committee whose honesty must be assumed, and the trust problem is displaced rather
than resolved.
...
McMahan et al.~\cite{mcmahan2017communication} introduced federated averaging (FedAvg),
the canonical federated learning protocol in which clients perform several epochs of
local training and a server aggregates the resulting weight vectors in proportion to
local sample counts. Data heterogeneity was identified early as a central obstacle:
Zhao et al.~\cite{zhao2018federated} showed that accuracy degrades substantially when
client distributions diverge, and FedProx~\cite{li2020federated} generalises FedAvg with
a proximal term that stabilises local updates under such heterogeneity. A complementary
line of work reduces the communication cost of update transmission through structured
and sketched updates~\cite{konecny2016federated}.

These protocols share an architectural constant: a central aggregation service assumed
to execute the aggregation rule faithfully. The server receives every update, computes
the global model, and distributes the result; nothing in the protocol allows clients to
establish that the returned model was actually computed from the submitted updates. In
IoT deployments where the aggregation infrastructure is operated by third parties --- or,
as in our setting, by an open set of heterogeneous compute nodes --- this assumption
becomes the weakest link in the pipeline.

\subsection{Byzantine-Robust Aggregation}

A second line of work hardens the aggregation \emph{rule} against malicious clients.
Krum~\cite{blanchard2017machine} selects the update with the smallest sum of squared
distances to its nearest neighbours, isolating outliers. Coordinate-wise trimmed mean
and median estimators~\cite{yin2018byzantine} achieve order-optimal statistical rates
while discarding extreme values in each dimension. Bulyan~\cite{elMhamdi2018hidden}
composes Krum-style selection with trimmed aggregation to resist attacks that hide
large perturbations in a single coordinate. FLTrust~\cite{cao2020fltrust} bootstraps
trust from a small server-side root dataset, weighting client updates by their
directional similarity to a server-computed reference update. Fang et
al.~\cite{fang2020local} showed that adaptive local model poisoning can nonetheless
degrade models protected by several of these defences, underlining that robust rules
are not a complete answer even for client-side threats.

All of these mechanisms are executed by the very entity whose honesty is in question:
a compromised aggregator may apply the rule selectively, apply it to a manipulated
subset of updates, or return an output unrelated to the submitted gradients.
Byzantine-robust rules are therefore orthogonal to this work. \tmfl{} currently
validates standard sample-weighted FedAvg, but the consensus-validated architecture is
rule-agnostic: a robust estimator could replace FedAvg inside the transaction processor
while preserving the same correctness guarantee.

\subsection{Blockchain-Based Federated Learning}

Blockchain has been applied to federated learning with several distinct aims ---
provenance, incentives, privacy, and decentralised coordination.

BlockFL~\cite{kim2020blockchained} replaces the central server with miners that
exchange, cross-verify, and record local updates in blocks; each device then computes
the global model locally from the recorded updates. Consensus attests to \emph{which
updates were recorded}, not to any aggregation result: a device or miner that
mis-computes the global model is not detected by the protocol, whose analysis focuses
on latency-optimal block generation and reward allocation.

BFLC~\cite{li2021bflc} elects a committee that scores incoming updates and produces
each round's global model, reducing consensus cost relative to network-wide validation;
correctness consequently rests on an honest-majority assumption over the elected
committee rather than on the full validator set. VFChain~\cite{peng2022vfchain}
similarly delegates aggregation to a committee, but additionally records verifiable
proofs of the committee's work on-chain, with an authenticated data structure for
efficient proof retrieval and secure committee rotation.

DeepChain~\cite{weng2021deepchain} targets gradient privacy and auditability: gradients
are encrypted under a threshold additively homomorphic scheme, and collateral deposits
with audit mechanisms deter deviation. Misbehaviour is deterred economically and
detected after the fact, rather than prevented by independent re-execution of the
aggregation computation.

Biscotti~\cite{shayan2021biscotti} is a fully peer-to-peer design in which
stake-weighted committees selected by verifiable random functions add differentially
private noise, filter updates with Multi-Krum, and assemble the global model through
secure aggregation over secret shares. Its guarantees require that a supermajority
(70\%) of stake is honest, and its evaluation on 20 cloud VMs with up to 200 peers
reported an order-of-magnitude slowdown relative to centralised federated learning.

BAFFLE~\cite{ramanan2020baffle} removes the aggregator entirely by holding the global
model in an Ethereum smart contract, partitioned into chunks that clients update through
a score-and-bid mechanism. Although contract execution is replicated across the
(private) chain's nodes, update scoring is computed off-chain by the clients themselves,
no model-quality validation is applied to what the contract stores, and gas limits bound
the size of on-chain model state.

In summary, existing systems either record FL artefacts on-chain while computing the
aggregate off-chain (BlockFL, DeepChain), delegate aggregation to a small committee
whose honesty must be assumed (BFLC, VFChain, Biscotti), or replicate contract
execution without validating the quality of the stored model (BAFFLE). \tmfl{} takes a
different position in this design space: the full sample-weighted FedAvg computation
\emph{and} a model-quality validation pipeline execute deterministically inside the
state-transition function of a BFT-replicated blockchain, so every PBFT validator
independently re-computes the aggregate from content-addressed weights and a round
commits only if the validators agree. Correctness of aggregation is thereby inherited
from the consensus fault model itself --- at most $f$ faulty validators out of
$3f{+}1$ --- rather than from a designated party, an elected committee, or economic
deterrence. To the authors' knowledge, \tmfl{} is the first system in which FedAvg
correctness is enforced as a consensus property and evaluated on a physical
heterogeneous IoT testbed. Table~\ref{tab:related} in
Section~\ref{sec:discussion} locates \tmfl{} within this design space.

\subsection{Blockchain-Enabled Distributed Computing for IoT}

Blockchain-integrated fog and edge computing frameworks provide the infrastructure
context for this work. FogBus~\cite{tuli2019fogbus} introduced blockchain-based data
integrity logging for fog computing, but retains a partially centralised control
structure. CoopEdge~\cite{yuan2021coopedge} builds a decentralised platform for
cooperative edge computing in which edge servers earn reputation recorded through
consensus. BlockEdge~\cite{kumar2020blockedge} couples permissioned and permissionless
chains in a three-tier industrial IoT architecture, evaluated in simulation. A
systematic comparison of these frameworks against \tm{} appears
in~\cite{rangwala2025trustmesh}. None of them supports federated learning, let alone
verifiable aggregation; \tmfl{} closes this gap by extending \tm{}'s consensus-backed
trust guarantees from task scheduling to the FL aggregation computation itself.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        flcomp/.style={draw=blue!60!black, fill=blue!12, rounded corners=1pt,
                       minimum height=4.5mm, inner sep=2pt, align=center},
        layer/.style={draw=black!50, rounded corners=2pt, inner sep=3pt},
        lname/.style={font=\scriptsize\bfseries, inner sep=1pt, anchor=south west},
    ...
    \node[lname] (mgmtlbl) at ($(depclient.north west)+(0,1mm)$) {Network Management Layer};
    \begin{scope}[on background layer]
        \node[layer, fit=(mgmtlbl)(depclient)(wfclient)] (mgmt) {};
    \end{scope}

    % ---- Computation Layer ----
    \node[comp, below=12mm of depclient.south west, anchor=north west] (validator) {Sawtooth Validator\\(PBFT)};
    ...
    \node[comp, below=of validator.south west, anchor=north west] (taskexec) {Task Executor\\(DinD)};
    ...
    \node[comp, below=of taskexec.south west, anchor=north west] (couchdb) {CouchDB Cluster\\(model\_weights)};
    \node[comp, right=of couchdb] (redis) {Redis\\Cluster};
    \node[comp, right=of redis] (peereg) {Peer Registry\\(membership)};
    \node[lname] (compllbl) at ($(validator.north west)+(0,1mm)$) {Computation Layer ($\times$4 compute nodes)};
    \begin{scope}[on background layer]
        \node[layer, fit=(compllbl)(validator)(aggconftp)(couchdb)(peereg)(valdist)] (compl) {};
    \end{scope}

    % ---- Perception Layer ----
    \node[comp, below=12mm of couchdb.south west, anchor=north west] (simulation) {FL Simulation\\(two-phase driver)};
    ...
    \node[lname] (perclbl) at ($(simulation.north west)+(0,1mm)$) {Perception Layer ($\times$4--10 IoT nodes)};
    \begin{scope}[on background layer]
        \node[layer, fit=(perclbl)(simulation)(txinit)] (perc) {};
    \end{scope}
...
\begin{figure*}[t]
    ...
    \begin{tikzpicture}[
        font=\scriptsize,
        actor/.style={draw, fill=black!8, rounded corners=1pt, minimum height=9mm,
    ...
    \draw[msg] ($(exec.south)+(0,-1.8)$)  -- ++(0.5,0) node[right, font=\tiny] {run training container};
...
\texttt{NON\_IID\_ALPHA}              & 0.5   & Dirichlet $\alpha$ \\
\texttt{AGGREGATION\_TIMEOUT}         & 180\,s & Collection window \\
\texttt{MIN\_NODES\_FOR\_AGGREGATION} & 1     & Minimum nodes to proceed \\
\texttt{MIN\_ACCURACY\_THRESHOLD}     & 0.30  & Accuracy floor $\tau_a$ \\
\texttt{MAX\_WEIGHT\_MAGNITUDE}       & 10.0  & Magnitude bound $\tau_w$ \\
\texttt{SKIP\_VALIDATION}             & false & Bypass validation checks \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{lllll}
\toprule
System & Aggregation computed by & Correctness mechanism & Update screening & Trust assumption \\
\midrule
BlockFL~\cite{kim2020blockchained}   & Each device, off-chain & None (updates recorded on-chain) & Miner cross-verification & Honest miner majority \\
BFLC~\cite{li2021bflc}               & Elected committee & Committee scoring & Committee validation scores & Honest committee majority \\
DeepChain~\cite{weng2021deepchain}   & Parties over encrypted gradients & Audit + collateral incentives & None (privacy focus) & Rational participants \\
Biscotti~\cite{shayan2021biscotti}   & VRF-selected committee & Secure aggregation, commitments & Multi-Krum & $\geq$70\% honest stake \\
VFChain~\cite{peng2022vfchain}       & Selected committee & Verifiable proofs on-chain & Committee verification & Honest committee majority \\
BAFFLE~\cite{ramanan2020baffle}      & Smart contract (chunked) & Replicated contract execution & Client-side score-and-bid & Honest chain quorum \\
\tmfl{} (this work)                  & All PBFT validators & Deterministic re-execution + model validation & Four-stage validation pipeline & $\leq f$ of $3f{+}1$ validators faulty \\
\bottomrule
\end{tabular}
\vspace{2pt}

{\footnotesize
\textit{Aggregation computed by}: the entity that produces the global model.
\textit{Correctness mechanism}: how the system establishes that the produced model is
the correct output of the aggregation rule.
\textit{Update screening}: defences applied to individual client updates.
}
\end{table*}
